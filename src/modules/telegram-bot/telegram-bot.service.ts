...
@Injectable()
    ...
    async sendLeads(ctx:Context) {
        ...
        await ctx.replyWithDocument(
            ...
            { caption: 'Добрый день! Высылаем Ваши лиды, которые заходили на сайты конкурентов вчера. Рекомендуем оперативно прозвонить лиды, поскольку они имеют короткий срок годности (1-2 дня с момента перехвата). Для касаний лидов Вы можете воспользоваться нашими инструментами: рассылки, обзвон, инвайтинг, настройка рекламы.' } 
        );
    }
