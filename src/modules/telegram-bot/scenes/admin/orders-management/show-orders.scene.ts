import { Injectable } from '@nestjs/common';
import { Ctx, Scene, SceneEnter, On, Command, Hears } from 'nestjs-telegraf';
import { Context } from 'telegraf';
import { SceneContext } from 'telegraf/typings/scenes';
import { UsersService } from 'src/modules/users/users.service';
import { ExelService } from 'src/modules/Exel-Module/exelModule.service';
import { TelegramBotService } from '../../../telegram-bot.service';
import { OrdersService } from 'src/modules/users/orders.service';
import { AdminService } from 'src/modules/admin/admin.service';
import { Markup } from 'telegraf';
import { BotMessages } from '../../../messages/messages';
import { addCancelButton, handleCancelButton } from '../../../helpers/scene.helper';

@Injectable()
@Scene('admin_show_orders')
export class ShowOrdersScene {
  constructor(
    private readonly usersService: UsersService,
    private readonly exelService: ExelService,
    private readonly telegramService: TelegramBotService,
    private readonly ordersService: OrdersService,
    private readonly adminService: AdminService
  ) {}

  @SceneEnter()
  async onSceneEnter(@Ctx() ctx: SceneContext) {
    const user = await this.usersService.getUserByTelegramId(ctx.from.id);

    if (user?.role !== 'admin' && user?.role !== 'moderator') {
      await ctx.reply('У вас нет прав для выполнения этого действия');
      return ctx.scene.leave();
    }

    try {
      const orders = await this.ordersService.getAllOrdersWithDetails() as any[];
      
      // Преобразуем данные для Excel
      const serializedOrders = orders.map(order => ({
        id: order.id?.toString(),
        userId: order.userId?.toString(),
        companyId: order.companyId?.toString(),
        maxCount: order.maxCount?.toString(),
        dailyCount: order.dailyCount?.toString(),
        projectName: order.projectName,
        username: order.username || 'Не указан',
        phone: order.phone || 'Не указан',
        company_name: order.company_name || 'Не указана',
        createdAt: order.createdAt?.toISOString(),
        updatedAt: order.updatedAt?.toISOString()
      }));

      const buffer = await this.exelService.exportToExcelBuffer(serializedOrders);

      await this.telegramService.sendDocumentBuffer(
        Number(ctx.from.id), 
        Buffer.from(buffer),
        { filename: 'orders.xlsx' }
      );
    } catch (error) {
      console.error('Ошибка при получении списка заказов:', error);
      await ctx.reply('Произошла ошибка при получении списка заказов');
    }

    await addCancelButton(ctx);
    return ctx.scene.leave();
  }

  @On('text')
  async onText(@Ctx() ctx: SceneContext) {
    const text = (ctx.message as any).text;
    
    if (await handleCancelButton(ctx, text)) {
      return;
    }

    // ... existing code ...
  }
} 